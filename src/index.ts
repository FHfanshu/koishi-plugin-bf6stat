...
function drawCardBackground(c: CanvasRenderingContext2D, width: number, height: number, accentColor: string) {
...
function drawHeaderSection(c: CanvasRenderingContext2D, stats: BattlefieldStats, options: RenderOptions, avatar: Image | null, rankImg: Image | null) {
  const paddingX = 56
  let baselineY = 96
...
function buildPrimaryMetrics(stats: BattlefieldStats): MetricBlock[] {
  return [
    {
      label: 'SPM',
    ...
    {
      label: '伤害',
      value: formatAbbreviated(stats.damage ?? stats.damageDealt),
      caption: '总伤害输出',
    },
  ]
}

function buildSecondaryMetrics(stats: BattlefieldStats): MetricBlock[] {
  return [
    {
      label: '最佳兵种',
    ...
    {
      label: '行程',
      value: formatDistance(stats.distanceTraveled ?? stats.distanceTravelled),
      caption: '战场位移',
    },
    {
      label: '总时间',
...
function drawMetricPanel(c: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, metric: MetricBlock, accentColor: string, emphasize = false) {
  c.save()
...
function drawFooter(c: CanvasRenderingContext2D, width: number, height: number) {
  c.save()
...
function roundRect(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, radiusTopLeft = 20, radiusTopRight = 20, radiusBottomRight = 20, radiusBottomLeft = 20) {
  ctx.beginPath()
...
function formatAbbreviated(value: number | undefined) {
  const num = Number(value)
...
function calcRate(part: number | undefined, total: number | undefined) {
...
function formatPlaytime(seconds: number | undefined) {
